...
<body class="antialiased">
        ...
        <div class="max-w-7xl mx-auto p-6 lg:p-8">
            ...
            <div class="mt-16">
                <div id="user-grid" class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                    ...
                    @empty
                        <p class="col-span-full text-center text-gray-600">No users found.</p>
                    @endforelse
                </div>
                <div id="load-more-container" class="mt-4 text-center">
                    @if($users->hasMorePages())
                        <button id="load-more-btn"
                                data-next-page="{{ $users->currentPage() + 1 }}"
                                data-next-url="{{ route('users.loadMore', ['page' => $users->currentPage() + 1]) }}"
                                class="pagination-item p-3 min-w-[48px] text-center
                ...
                @if ($users->lastPage() > 1)
                    <div class="mt-4 flex justify-center items-center space-x-2">
                        @if ($users->onFirstPage())
                            <span class="pagination-item p-3 min-w-[48px] text-center bg-gray-300 text-gray-500 cursor-not-allowed
                                            rounded-full shadow-2xl shadow-gray-500/20">
                                Prev
                            </span>
                        @else
                            <a href="{{ $users->previousPageUrl() }}"
                               class="pagination-item p-3 min-w-[48px] text-center
                                        bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent
                                        dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-full shadow-2xl shadow-gray-500/20
                        ...
                        @for ($page = 1; $page <= $users->lastPage(); $page++)
                            @if ($page == $users->currentPage())
                                <span class="pagination-item p-3 min-w-[48px] text-center
                                          bg-white bg-yellow-800/50 bg-gradient-to-bl from-yellow-700/50 via-transparent
                                          dark:ring-1 dark:ring-inset dark:ring-yellow/5 rounded-full shadow-2xl shadow-gray-500/20
                                          text-gray-700 dark:text-gray-300 hover:bg-yellow-100 dark:hover:bg-gray-700
                                          transition-colors duration-195">
    ...
    <div id="app" data-users-store-route="{{ route('users.store') }}">
    </div>
